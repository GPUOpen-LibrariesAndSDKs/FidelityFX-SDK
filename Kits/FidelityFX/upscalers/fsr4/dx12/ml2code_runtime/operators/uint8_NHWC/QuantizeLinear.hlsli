    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    for (uint y = 0 ; y < perThreadWork.y; y++)
        ...
        {
            int3 poBase = workOffset + uint3(x, y, 0);
            ...
            for (uint c = 0 ; c != perThreadWork.z; c += 4)
            {
                ...
                const uint8_t4_packed packed = pack_clamp_u8(int4(vs * rcpScale));
                Store4u8A(output, po, packed);
            }
        }
}
    ...
    for (uint y = 0 ; y < perThreadWork.y; y++)
        for (uint x = 0; x < perThreadWork.x; x++)
        {
            ...
            for (uint c = 0 ; c != perThreadWork.z; c += 4)
            {
                ...
                const uint8_t4_packed packed = pack_clamp_u8(int16_t4(vs * rcpScale));
                Store4u8A(output, po, packed);
            }
        }
}