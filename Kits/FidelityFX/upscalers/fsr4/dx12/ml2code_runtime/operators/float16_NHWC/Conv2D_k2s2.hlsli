    ...
    const Tensor3h_NHWC<SO> output)
{
    if (input.logicalSize.z == 16 && numFeatures == 32)
        return InnerConv4_k2s2_16_32(piBase, poBase, numFeatures, input, weights, output);
...
template<typename SI, typename SW, typename SO>
void InnerConv4_k2s2Padded_16_32(
    const int3 piBase,
    const int3 poBase,
    const uint numFeatures,
    const Tensor3h_NHWC<SI> input,
...
{
    if (input.logicalSize.z == 16 && numFeatures == 32)
        return InnerConv4_k2s2Padded_16_32(piBase, poBase, numFeatures, input, weights, output);
...
template<typename SI, typename SW, typename SO>
void InnerConv4_k2s2PaddedGrouped_32_32(
    const int3 piBase,
    const int3 poBase,
    const uint numFeatures,
...
template<typename SI, typename SW, typename SO>
void InnerConv4_k2s2PaddedGroups(
    const int3 piBase,
    const int3 poBase,
    const uint numFeatures,
    const uint numGroups,
    ...
    const Tensor3h_NHWC<SO> output)
{
    if (input.logicalSize.z == 32 && numFeatures == 32)
        return InnerConv4_k2s2PaddedGrouped_32_32(piBase, poBase, numFeatures, numGroups, input, weights, output);
    ...
    for (uint y = 0 ; y < perThreadWork.y; y++)
        for (uint x = 0 ; x < perThreadWork.x; x++)
        {
            ...
            if (IsAligned(numFeatures, 4))
                InnerConv4_k2s2(piBase, poBase, numFeatures, input, weights, output);
        }
}
