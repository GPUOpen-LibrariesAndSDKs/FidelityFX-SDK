    ...
    const Tensor3h_NHWC<SO> output)
{
    if (input.logicalSize.z == 16 && numFeatures == 32)
        return InnerConv4_16_32_k1b(piBase, poBase, numFeatures, input, weights, output);
    ...
    const Tensor3h_NHWC<SO> output)
{
    if (input.logicalSize.z == 8 && numFeatures == 16)
        return InnerConv4_8_16_k1b(piBase, poBase, numFeatures, input, weights, bias, output);

    if (input.logicalSize.z == 10 && numFeatures == 16)
        return InnerConv4_10_16_k1b(piBase, poBase, numFeatures, input, weights, bias, output);

    if (input.logicalSize.z == 16 && numFeatures == 32)
        return InnerConv4_16_32_k1b(piBase, poBase, numFeatures, input, weights, bias, output);

    if (input.logicalSize.z == 32 && numFeatures == 64)
        return InnerConv4_32_64_k1b(piBase, poBase, numFeatures, input, weights, bias, output);
    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    {
        Conv2dDownsample_2x2_UnRolled2x(1,
                            1,
                            input,
                            weights,
                            bias,
    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    TemplatedConv2D_3413_NHWC_44<half4, half4, half4>(algorithm, 0, 0, 0, 0, 1, 1, output.threadGroupSliceStart, output.threadGroupSliceSize,
        computeShaderParams);
}
...
void Conv2D(
    ...
    const ComputeShaderParams computeShaderParams)
{
    // TODO: this combinations blows out the representable range of half so now uses floats
    ...
    struct Algorithm {
        ...
        float4 LoadInput4(int3 p) { return unpack_u8u16(Load4u8A(input, p)) * inputScale; }
        float4 LoadWeight4(int4 p) { return unpack_s8s16(Load4i8A(weights, p)) * weightsScale;  }
        float Dot(float4 ws, float4 vs, float acc) { return dot(ws, vs) + acc; }
        void StoreOutput4(int3 p, float4 vs) { Store4hA(output, p, vs); }
    };
    ...
    const uint numFeatures)
{
    int accumulator[32];
    [unroll]
    for (uint f = 0 ; f != 32; f++) { accumulator[f] = 0; }
    ...
    [unroll]
    for (uint f = 0; f != numFeatures; f+=8)
    {
        int3 po = poBase;
        po.z += f;
        ...
        half4 result = acc * weights.quantizationScale * input.quantizationScale + Load4i32(bias, f) * bias.quantizationScale;
        storageBytes.xy = Pack4h(result);

        acc = int4(accumulator[f+4], accumulator[f+5], accumulator[f+6], accumulator[f+7]);
        result = acc * weights.quantizationScale * input.quantizationScale + Load4i32(bias, f+4) * bias.quantizationScale;
        storageBytes.zw = Pack4h(result);
    ...
    const uint numFeatures)
{
    ...
    for (uint ky = 0; ky < weights.logicalSize.y; ++ky)
    {
        ...
        for (uint kx = 0; kx < weights.logicalSize.x; ++kx)
        {
            ...
            vs[nInput++] = inputDwords.x;
            vs[nInput++] = inputDwords.y;
            vs[nInput++] = inputDwords.z;
            vs[nInput++] = inputDwords.w;
    ...
    for (uint f = 0 ; f < numFeatures; )
    {
        ...
        for (uint ky = 0; ky < weights.logicalSize.y; ++ky)
        {
            ...
            for (uint kx = 0; kx < weights.logicalSize.x; ++kx)
            {
                ...
                {
                    const uint weightsOffset = weights.OffsetOf(uint4(kx, ky, c, f));
                    const uint4 weightDwords = weights.storage.Load4(weightsOffset);

                    accumulator[f%4] = dot4add_i8packed(weightDwords.x, vs[nInput++], accumulator[f%4]);
                    accumulator[f%4] = dot4add_i8packed(weightDwords.y, vs[nInput++], accumulator[f%4]);
                    accumulator[f%4] = dot4add_i8packed(weightDwords.z, vs[nInput++], accumulator[f%4]);
                    accumulator[f%4] = dot4add_i8packed(weightDwords.w, vs[nInput++], accumulator[f%4]);
                }
            }
        }
        ...
        if (!(f % 4))
        {
            ...
            const half4 result = accumulator * weights.quantizationScale * input.quantizationScale + Load4i32(bias, f-4) * bias.quantizationScale;
            Store4hA(output, po, result);
        }
    }
}
    ...
    const ComputeShaderParams computeShaderParams)
{
    struct Algorithm {
        ...
        int4 LoadBias4(int p) { return 0; }
        int8_t4_packed LoadInput4(int3 p) { return (Load4i8A(input, p)); }
        int8_t4_packed LoadWeight4(int4 p) { return (Load4i8A(weights, p));  }

        int Dot(int8_t4_packed ws, int8_t4_packed vs, int acc)
        {
            return dot4add_i8packed(ws, vs, acc);
        }

        void StoreOutput4(int3 p, int4 vs)
        {
            half4 result = vs * weightsScale * inputScale + Load4i32(bias, p.z) * biasScale;
            Store4hA(output, p, result);
        }
    };
    ...
    if (numFeatures == 32 && input.logicalSize.z == 16)
    {
        ...
        for (uint y = 0 ; y < perThreadWork.y; y++)
            for (uint x = 0 ; x < perThreadWork.x; x++)
            {
                ...
                if (IsAligned(numFeatures, 4))
                    Conv_16_32(input, weights, bias, output, piBase, poBase, numFeatures);
            }
    }
    else if (numFeatures == 64 && input.logicalSize.z == 32)
    {
        ...
        for (uint y = 0 ; y < perThreadWork.y; y++)
            for (uint x = 0 ; x < perThreadWork.x; x++)
            {
                ...
                if (IsAligned(numFeatures, 4))
                    Conv_32_64(input, weights, bias, output, piBase, poBase, numFeatures);
            }
    }
    else
...
//         float4 LoadBias4(int p) { return 0; }
//         half4 LoadInput4(int3 p) { return unpack_s8s16(Load4i8A(input, p)) * inputScale; }
//         half4 LoadWeight4(int4 p) { return unpack_s8s16(Load4i8A(weights, p)) * weightsScale;  }
...
//         {
//             const half4 result = vs + Load4i32(bias, p.z) * biasScale;
//             Store4hA(output, p, result);
//         }
//     };
