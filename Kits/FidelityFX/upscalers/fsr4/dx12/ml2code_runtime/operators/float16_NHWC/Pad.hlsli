...
template<typename TI, typename SC, typename SO>
void SetPaddingConstant(
    const TI input,
    const Tensor1h<SC> constant_value,
    ...
    const uint3 paddingEnd)
{
    ...
    ResetPaddingFloat16(output, computeShaderParams.dispatchThreadID, paddingBegin.x > 0 || paddingEnd.x > 0, paddingBegin.y > 0 || paddingEnd.y > 0, stride.xy);
}
    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    for (uint y = 0; y < perThreadWork.y; y++)
        for (uint x = 0; x < perThreadWork.x; x++)
        {
            ...
            if (any(poBase.xy < paddingBegin.xy | poBase.xy >= output.logicalSize.xy - paddingEnd.xy))
                continue;
            ...
            {
                int8_t4_packed values = Load4i8A(input, poBase + int3(0, 0, c) - paddingBegin);
                const half4 dequantized = unpack_s8s32(values) * input.quantizationScale;
                Store4hA(output, poBase + int3(0, 0, c) , dequantized);
            }
        }
    ...
    Tensor3h_NHWC <SO> output_reshaped = output;
    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    for (uint y = 0; y < perThreadWork.y; y++)
        for (uint x = 0; x < perThreadWork.x; x++)
        {
            ...
            for (uint c = 0; c < input.logicalSize.z; c += 4) //Assume we are memory padding to multiples of four for int8
            {
                half4 values = Load4hA(input, poBase + int3(0, 0, c));
                Store4hA(output, poBase + int3(0, 0, c) + paddingBegin, values);
            }
        }
    ...
    ComputeShaderParams computeShaderParams)
{
    ...
    const Tensor1h< ConstantBufferStorage <1> > _Constant_Zero_Dword = {
        1, // logicalSize
