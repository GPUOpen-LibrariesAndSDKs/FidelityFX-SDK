...
void InnerConv4_16_48(
    ...
    const Tensor3h_NHWC<SO> output)
{
    half accumulators[48];
    uint biasOffset = bias.OffsetOf(0);

    [unroll]
    for (uint f = 0 ; f != 48; f+=8)
    {
        const uint4 biasDwords = bias.storage.Load4(biasOffset);
        half4 b = Unpack4h(biasDwords.xy);
...
#define UNROLL_NUM 8
    ...
    for (int OuterFeatureId = 0; OuterFeatureId < NumOuterIterations; ++OuterFeatureId)
    {
        ...
        for (int UnRollId = 0; UnRollId < UNROLL_NUM; ++UnRollId)
        {
            ...
            {
#ifdef WAVE_64
                Accumulator[UnRollId].x += WaveReadLaneAt(Accumulator[UnRollId].x, (ThreadIndex ^ (1u << WaveOpId)));
#else
                Accumulator[UnRollId].x += WaveReadLaneAt(Accumulator[UnRollId].x, (ThreadIndex + (1u << WaveOpId)));
#endif
            }
    ...
    for (int OuterFeatureId = 0; OuterFeatureId < NumOuterIterations; ++OuterFeatureId)
    {
        ...
        for (int UnRollId = 0; UnRollId < UNROLL_NUM; ++UnRollId)
        {
            ...
            for (int BlockId = 0; BlockId < NUM_BLOCKS; ++BlockId)
            {
                ...
                {
#ifdef WAVE_64
                    Accumulator[BlockId][UnRollId].x += WaveReadLaneAt(Accumulator[BlockId][UnRollId].x, (ThreadIndex ^ (1u << WaveOpId)));
#else
                    Accumulator[BlockId][UnRollId].x += WaveReadLaneAt(Accumulator[BlockId][UnRollId].x, (ThreadIndex + (1u << WaveOpId)));
#endif
                }
...
#ifdef WAVE_64
                    uint NearbyPackedAccumulator = WaveReadLaneAt(PackedAccumulator, (ThreadIndex ^ (1u << WaveOpId)));
#else
                    int NearbyPackedAccumulator = WaveReadLaneAt(PackedAccumulator, (ThreadIndex + (1u << WaveOpId)));
#endif
    ...
    const Tensor3h_NHWC<SO> output)
{
    ...
    if (input.logicalSize.z == 16 && numFeatures == 48)
        return InnerConv4_16_48(piBase, poBase, numFeatures, input, weights, bias, output);

    if (input.logicalSize.z == 32 && numFeatures == 64)
        return InnerConv4_32_64(piBase, poBase, numFeatures, input, weights, bias, output);

    if (input.logicalSize.z == 48 && numFeatures == 64)
        return InnerConv4_48_64(piBase, poBase, numFeatures, input, weights, bias, output);
    half4 accumulator;
    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    {
        Conv2dDownsample_2x2_UnRolled2x(2,
                            2,
                            input,
                            weights,
                            bias,
    ...
    const ComputeShaderParams computeShaderParams)
{
    ...
    TemplatedConv2D_3413_NHWC_44<half4, half4, half4>(algorithm, 0, 0, 0, 0, 2, 2, output.threadGroupSliceStart, output.threadGroupSliceSize,
        computeShaderParams);
}
