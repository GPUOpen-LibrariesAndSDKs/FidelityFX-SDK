...
void ConvTranspose2D_2x2_2x2_3413_NHWC_HWCN_44i8(
...
void FusedConvTranspose2D_k2s2b_QuantizedOutput(
    const float quantScale0,
    const float quantScale1,
    const QuantizedTensor3i8_NHWC<SI> input,
    const QuantizedTensor4i8_HWCN<SW> weights,
    ...
    const ComputeShaderParams computeShaderParams)
{
     if (weights.logicalSize.w == 48)
        return ConvTranspose2D_2x2_2x2_3413_NHWC_HWCN_44i8<48>(quantScale0, quantScale1, input, weights, bias, output, computeShaderParams);
}