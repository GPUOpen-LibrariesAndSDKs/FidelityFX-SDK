    ...
    const ComputeShaderParams computeShaderParams)
{
    if (weights.logicalSize.w == 16)
        return ConvTranspose2D_Add_2x2_2x2_3413_NHWC_HWCN_44i8<16>(input, inputAdd, weights, bias, output, computeShaderParams);

    if (weights.logicalSize.w == 32)
        return ConvTranspose2D_Add_2x2_2x2_3413_NHWC_HWCN_44i8<32>(input, inputAdd, weights, bias, output, computeShaderParams);

    if (weights.logicalSize.w == 64)
        return ConvTranspose2D_Add_2x2_2x2_3413_NHWC_HWCN_44i8<64>(input, inputAdd, weights, bias, output, computeShaderParams);
}